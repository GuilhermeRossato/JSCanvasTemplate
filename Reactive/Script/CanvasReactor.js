/*	CanvasReactor.js - Created by GuilhermeRossato 10/2016
 *
 ...
 *  = new CanvasReactor(document.getElementById("recipient"));
 * --------------------------------------------------------------------------------------------------------
 * Methods:
 *	constructor(recipient[, width, height, ctxMenu])	---> Class Constructor ( new CanvasReactor(...) )
 ...
 *
 *	constructor(canvas[, width, height, ctxMenu])	---> Class Constructor ( new CanvasReactor(...) )
 *		canvas		(HTMLCanvasElement)	The <canvas> element you want to be associated and handled by this class
 *
 *	constructor(str[, width, height, ctxMenu])	---> Class Constructor ( new CanvasReactor(...) )
 ...
 * 		type		(string)			Candidates: "draw", "mousemove", "mousedown", "mouseup", "mouseclick", "keydown", "keyup".
...
function CanvasReactor(arg1, width, height, ctxMenu) {
	...
	if (document instanceof Document) {
		...
		document.addEventListener("mousedown", ev=>CanvasReactor.prototype.onMouseDown.call(self, ev), false);
		document.addEventListener("mouseup", ev=>CanvasReactor.prototype.onMouseUp.call(self, ev), false);
		document.addEventListener("mousemove", ev=>CanvasReactor.prototype.onMouseMove.call(self, ev), false);
		document.onkeydown = (ev=>CanvasReactor.prototype.onKeyDown.call(self, ev));
		document.onkeyup = (ev=>CanvasReactor.prototype.onKeyUp.call(self, ev));
		document.addEventListener("keyup", ev=>CanvasReactor.prototype.onKeyUp.call(self, ev), false);
	} else {
...
CanvasReactor.prototype = {
	constructor: CanvasReactor,
	eventCandidates: ["draw", "mousemove", "mousedown", "mouseup", "mouseclick", "keydown", "keyup"],
	draw: function() {
		if (this.inProcess) {
	...
	onMouseDown: function(ev) {
		...
		let self = this
		  ...
		  , btnCode = ev.button;
		if ((!(this.events["mousedown"]instanceof Array)) || (this.events["mousedown"].every(obj=>obj.call(this, btnCode, m.x, m.y)))) {
			switch (btnCode) {
			case 0:
	...
	onMouseUp: function(ev) {
		...
		let self = this
		  ...
		  , m = this.getMousePosition(ev)
		  , btnCode = ev.button;
		if ((!(this.events["mouseup"]instanceof Array)) || (this.events["mouseup"].every(func=>func.call(self, btnCode, m.x, m.y)))) {
			switch (btnCode) {
			case 0:
		...
		if ((!(this.events["mousemove"]instanceof Array)) || (this.events["mousemove"].every(func=>func.call(self, m.x, m.y)))) {
			this.objects.forEach(function(obj) {
				if (obj instanceof Object && obj.alive !== false) {
					if (obj.onMouseMove instanceof Function && obj.onMouseMove.call(obj, m.x, m.y))
						redraw = true;
		...
		addEventListener: function(type, listener) {
			if (typeof (type) === "string") {
				...
				if (type.substr(0, 2) == "on")
					...
					type = type.substr(2);
				var id = CanvasReactor.prototype.eventCandidates.indexOf(type);
				if (id !== -1) {
